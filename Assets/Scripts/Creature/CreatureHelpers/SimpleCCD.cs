...
public class SimpleCCD : MonoBehaviour
{
	...
	void Start()
	{
        ...
        if (creature = gameObject.GetComponentInParent<Creature>())
            this.facingRight = creature.facingRight;
    }
	...
	float ClampAngle (float angle, float min, float max)
	{
        ...
        if (!facingRight && CheckFacingRight)
        {
            min -= 180; max -= 180;
        }
        print(min + " " + max + " "+ facingRight);
